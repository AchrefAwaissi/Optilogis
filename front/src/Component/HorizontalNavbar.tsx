...
const HorizontalNavbar: React.FC<HorizontalNavbarProps> = ({ user, onLogout, onSignInClick, onSignUpClick }) => {
    ...
    <>
      ...
      {user && (
        <>
          ...
          <button
            onClick={handleFavoritesClick}
            className="text-gray-500 hover:text-red-500 font-bold p-2 rounded flex items-center justify-center mr-5"
            aria-label="Favorites"
          >
            <FontAwesomeIcon icon={faHeart} className="text-[#095550]" />
          </button>
          <button onClick={onLogout} className="block mt-4 w-full text-left text-gray-800 hover:text-blue-600 mr-4">
            <FontAwesomeIcon icon={faSignOutAlt} className="mr-2 text-[#095550]" />
            Logout
          </button>
        </>
  ...
  return (
    <>
      {/* Conditionally render navbar if the user is logged in */}
      {user && (
        <header className="bg-white shadow-md fixed top-0 left-0 w-full z-50">
          <div className="container mx-auto px-4">
            <div className="flex items-center justify-between h-16">
              ...
              <div className="flex items-center bg-gray-100 rounded-lg w-[370px] h-[47px] pl-3">
